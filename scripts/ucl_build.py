#!/usr/bin/env python3
"""Build UEFA Champions League player leaderboards (appearances / goals /
assists), mirroring scripts/wc26_build.py.
...
Photos follow the same Transfermarkt-first pipeline as the WC26 page and share
one tid -> photo cache (scripts/imgcache.py). Unlike national teams, clubs don't
map to a pes.db country, so instead of a nation lookup we flatten pes.db into a
single name -> Transfermarkt-id map: most Champions League players are also
internationals whose ids (and often photos) we already know for free.
"""
...
SCOREBOARD = BASE + "/scoreboard?dates={start}-{end}&limit=400"
SUMMARY = BASE + "/summary?event={id}"
# 2025-26 season window (league phase kickoff through the final). Override with
# UCL_START / UCL_END env vars when a new season starts.
UCL_START = os.environ.get("UCL_START", "20250916")
UCL_END = os.environ.get("UCL_END", "20260603")
HEADSHOT = "https://a.espncdn.com/i/headshots/soccer/players/full/{id}.png"
TEAM_LOGO = "https://a.espncdn.com/i/teamlogos/soccer/500/{id}.png"
...
TM_DAILY_LIMIT = int(os.environ.get("UCL_TM_LIMIT", "26"))
TM_NULL_LIMIT, TM_ERROR_LIMIT, SEARCH_MISS_LIMIT = 3, 5, 3

HERE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.dirname(HERE)
OUT_DIR = os.path.join(ROOT, "assets", "ucl")
...
def pes_name_index():
    ...
    country, so we match Champions League players by name alone; names that map
    to more than one id are dropped to avoid mixing up different players."""
...
def completed_events():
    data = get_json(SCOREBOARD.format(start=UCL_START, end=UCL_END))
    out = []
...
def attach_images(players):
    ...
    # eFootball and WC26 builders, so photos resolved by any page are reused.
